...
Class staticMapLite
{
    ...
    protected $markerPrototypes = array(
        'default' => array('regex' => '/^marker-icon$/',
            'extension' => '.png',
            'shadow' => false,
            'offsetImage' => '-12,-40',
            'offsetShadow' => false
    ...
    public function parseLiteParams()
    {
        ...
        if (!empty($_GET['markers'])) {
            ...
            foreach ($markers as $marker) {
                ...
                $markerType = 'ol-marker';//basename($markerType);
	...
	if (!empty($_GET['mlat0'])) {
            ...
            if (!empty($_GET['mlon0'])) {
                ...
                $this->markers[] = array('lat' => $markerLat, 'lon' => $markerLon, 'type' => "ol-marker");
            }
        }
    }
    ...
    public function placeMarkers()
    {
        ...
        foreach ($this->markers as $marker) {
            ...
            if ($markerFilename == '' || !file_exists($this->markerBaseDir . '/' . $markerFilename)) {
                $markerIndex++;
                $markerFilename = 'marker-icon' . $markerIndex . '.png';
                $markerImageOffsetX = -12;
                $markerImageOffsetY = -40;
            }
            ...
            if (file_exists($this->markerBaseDir . '/' . $markerFilename)) {
                ...
                $markerImg = imagecreatefrompng($this->markerBaseDir . '/marker-icon.png');
            }
    ...
    public function fetchTile($url)
    {
        ...
        //} else {
        	curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Linux; Android 7.1.2; Nexus 7 Build/NZH54D; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/52.0.2743.100 Safari/537.36');
      	//}
